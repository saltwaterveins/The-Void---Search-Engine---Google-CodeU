...
public class WIkiParser {
	...
	private boolean validLink(Element elt) {
		...
		if (startsWith(elt, "/wiki/Wikipedia:")) {
			return false;
		}
		if (startsWith(elt, "/wiki/User")) {
			return false;
		}
		if (startsWith(elt, "/wiki/Template:")) {
			return false;
		}
		if (!(isInCorrectDiv(elt))) {
			return false;
		}
		if (alreadyBeen.contains(elt)) {
			return false;
		}
	...
	private boolean isInCorrectDiv(Element elt) {
		return (elt.parentNode().parentNode().parentNode().attr("class").equals("div-col columns column-count column-count-3"));
